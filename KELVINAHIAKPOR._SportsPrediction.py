...
df2 = pd.DataFrame(imputer_pipeline.fit_transform(df2), columns=df2.columns)
df2
...
df = df[df.duplicated(subset=['player_id'], keep=False)]

df2 = df2[df2.duplicated(subset=['player_id'], keep=False)]
df
...
playersNeeded = pd.concat([df, df2], axis=1)
...
# 
# Now we will move on, selecting only these 9 features (including value) and 1 label to train our model.

# In[21]:


dfP = dfP[most_correlated_cols].copy()
dfP.loc[:, 'value_eur'] = players.loc[df.index, 'value_eur'].values
dfP
...
X_scaled = imputer_pipeline.fit_transform(X_scaled)
...
X = pd.DataFrame(imputer_pipeline.fit_transform(X), columns=X.columns)
X
...
# 
# [8]Jacob Tadesse. 2020. How to Spit a Pickled Model File to Bypass Upload Limits on PythonAnywhere. Scribe.rip. Retrieved June 23, 2024 from https://scribe.rip/how-to-spit-a-pickled-model-file-to-bypass-upload-limits-on-pythonanywhere-e051ea1cec2d
